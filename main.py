...
async def get_current_user(db=Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    if user is None:
        raise credentials_exception
    return dict(user)  # Convert asyncpg.Record to dict
...
async def register_user(user: UserCreate, db=Depends(get_db)):
    ...
    return dict(new_user)  # Convert to dict
...
async def read_graduates(
    ...
    if highest_qualification:
        query += f" AND highest_qualification = ${len(params) + 1}"
        params.append(highest_qualification)
    
    if job_field:
        query += f" AND job_field = ${len(params) + 1}"
    ...
    return [dict(g) for g in graduates]  # Convert list of Records to dicts
